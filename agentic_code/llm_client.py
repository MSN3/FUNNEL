...
class GlobalRateLimiter:
    ...
    async def wait_for_token(self, provider_name: str):
        # Lazy initialization: Create the lock the first time it's needed
...
gcp_rate_limiter = GlobalRateLimiter(50)
    ...
    async def invoke(self, system_prompt: str, user_prompt: str, temperature: float = 0.0, max_tokens: int = 1024) -> Tuple[str, Dict[str, int]]:
        ...
        for attempt in range(max_retries):
            try:
                # --- vLLM / OPENAI LOGIC ---
                if self.provider in ["vllm", "openai"]:
                    # Force JSON instruction for vLLM models that don't support system roles well
                    combined_system = (
