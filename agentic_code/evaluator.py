...
def run_evaluation_and_plotting(results_df: pd.DataFrame, model_name: str):
    ...
    frugality_ratio = results_df['frugality_ratio'].mean()
    frugality_index = results_df['FI_New'].mean()
    # Calculate 95% confidence interval for frugality index
    frugality_ratio_ci = stats.t.interval(0.95, len(results_df) - 1, loc=frugality_ratio, scale=stats.sem(results_df['frugality_ratio']))
    # Calculate 95% confidence interval for accuracy
    accuracy_ci = stats.t.interval(0.95, len(results_df) - 1, loc=accuracy, scale=stats.sem(results_df['is_correct']))
    # accuracy_std_err = math.sqrt((accuracy * (1 - accuracy)) / total_patients) if total_patients > 0 else 0
    frugality_index_ci = stats.t.interval(0.95, len(results_df) - 1, loc=frugality_index, scale=stats.sem(results_df['FI_New']))
    ...
    summary_metrics = {
        ...
        "Accuracy_Percent": f"{accuracy*100:.2f}% (± {accuracy_ci[1]*100:.2f}%)",
        "Frugality_Index": f"{frugality_index:.2f} (± {frugality_index_ci[1]*100:.2f}%)",
        "Frugality_Ratio": f"{frugality_ratio:.2f} (± {frugality_ratio_ci[1]*100:.2f}%)",
